...
- POST /api/tasks?type=inv_check&sid= 总可用库存核对（FBA需求表 + 库存表，按 SKU编码 比对本地SKU总可用库存与可用库存）
...
def process_file(in_path, w3, w7, w14, out_path, cols=None, on_progress=None, cfg=None, mapping=None):
    ...
    def _compute_rules_m(row, store, asin):
        ...
        if not rules_list:
            rules_list = [rules_by_name.get(default_rule_name)] if default_rule_name else None
...
def process_ops(in_ops, in_sales_raw, out_path, w3, w7, w14, cols=None, on_progress=None, levels=None, cfg=None, mapping=None):
    """运营级别处理：按店铺ASIN 从销量原始文件直接 lookup 3/7/14/30/60/90 天日均销量，
    同店铺ASIN 下多条正常销售 MSKU 的销量相加；输出表头增加这 6 列，并保留加权销量与满足条件标记。
    - legacy 模式：加权销量 = (w3*d3 + w7*d7 + w14*d14) / (w3+w7+w14)，按 店铺ASIN 汇总。
    - rules 模式：按运营级别绑定规则计算加权销量，可展开 [规则,天数,满足条件,备注]。

    参数：
      w3/w7/w14 : legacy 模式加权权重（rules 模式下仅用于总进度/汇总展示，不参与规则计算）。
      cfg       : 备货规则配置（mode=='rules' 时生效）
      mapping   : 运营级别映射表 {店铺ASIN: (运营级别, 补货方式)}
    """
    C = dict(DEF_OPS_COLS)
    if cols:
        for k, v in cols.items():
    ...
    def fidx(hdr, name):
        for i, h in enumerate(hdr):
            if h == name:
                return i
        return -1
    def win_idx_for(hdr, win):
        ov = (cols or {}).get('d%d' % win) if cols else None
        name = ov or WIN_COLS[win]
        return fidx(hdr, name)

    # 1) 读销量原始文件，按 店铺ASIN 预聚合 6 窗口销量（仅正常销售）
    sh, sit = _calamine_rows(in_sales_raw) if _try_calamine(in_sales_raw) else _openpyxl_rows(in_sales_raw)
    i_store = fidx(sh, C['store']); i_asin = fidx(sh, C['asin'])
    i_stat = fidx(sh, '状态')
    win_idx = {w: win_idx_for(sh, w) for w in WIN_COLS}
    # 3/7/14 必须存在；30/60/90 若缺失按 0 处理
    if i_store < 0 or i_asin < 0:
        raise ValueError("销量原始文件缺少必要列：%s / %s（当前列：%s）" % (C['store'], C['asin'], sh))
    if win_idx[3] < 0 or win_idx[7] < 0 or win_idx[14] < 0:
        raise ValueError("销量原始文件缺少必要列：三天日均销量 / 七天日均销量 / 十四天日均销量（当前列：%s）" % sh)

    # 6 窗口汇总：{店铺ASIN: {3:sum, 7:sum, 14:sum, 30:sum, 60:sum, 90:sum}}
    sales_6win = {}
    # legacy 加权汇总
    sales_weighted = {}
    ...
    for r in sit:
        ...
        if i_stat >= 0 and r[i_stat] is not None and str(r[i_stat]).strip() != '正常销售':
            continue
        # 6 窗口累计
        sums = sales_6win.get(a)
        if sums is None:
            sums = {}
            sales_6win[a] = sums
        for w, ci in win_idx.items():
            if ci >= 0:
                sums[w] = sums.get(w, 0.0) + fnum(r[ci])
        # legacy 加权
        if not use_rules:
    ...
    # 清洗表头：去掉名为空的列；若原表已有 店铺ASIN/销量/6窗口列 则不重复添加
    clean_idx = [i for i, h in enumerate(oh) if h != '']
    clean_oh = [oh[i] for i in clean_idx]
    i_key2 = fidx(clean_oh, '店铺ASIN')
    iL2 = fidx(clean_oh, '销量')
    # 6 窗口原始销量列名（输出用）
    WIN_OUT_COLS = ['三天日均销量', '七天日均销量', '十四天日均销量', '三十天日均销量', '60天日均销量', '90天日均销量']
    existing_win = [c for c in WIN_OUT_COLS if fidx(clean_oh, c) >= 0]
    prepend_a = (i_key2 < 0)

    extra_tail = list(WIN_OUT_COLS)
    if use_rules:
        if iL2 < 0:
            extra_tail = extra_tail + ['销量', '规则', '天数', '满足条件', '备注']
        else:
            extra_tail = extra_tail + ['规则', '天数', '满足条件', '备注']
    else:
        if iL2 < 0:
            extra_tail = extra_tail + ['销量', '满足条件']
        else:
            extra_tail = extra_tail + ['满足条件']

    out_header = list(clean_oh)
    if prepend_a:
        out_header = ['店铺ASIN'] + out_header
    out_header = out_header + extra_tail
    ...
    def row_gen():
        ...
        for r in oit:
            ...
            # 6 窗口原始销量（从销量原始文件直接 lookup，同店铺ASIN 多条 MSKU 已相加）
            win_vals = sales_6win.get(a, {})
            win_part = [round(win_vals.get(w, 0.0) * 10000) / 10000 for w in WIN_COLS]

            if not use_rules:
                # ---- legacy：单行 + 白名单 ----
                L = sales_weighted.get(a, 0.0)
                ...
                if ok:
                    meet += 1
                if iL2 < 0:
                    cr = cr_base + win_part + [round(L * 10000) / 10000]
                else:
            ...
            if not rules_list:
                ...
                if iL2 < 0:
                    cr = cr_base + win_part + [0, '', '', '否', '未绑定规则(运营级别未选)']
                else:
                    cr_base[sales_idx] = 0
                    cr = cr_base + win_part + ['', '', '否', '未绑定规则(运营级别未选)']
                yield cr
                n += 1
                if on_progress and n % 5000 == 0:
                    on_progress(n)
            ...
            for rule in rules_list:
                ...
                if ok:
                    meet += 1
                if iL2 < 0:
                    cr = cr_base + win_part + [M_rule, rule['name'], days, '是' if ok else '否', note]
                else:
                    tmp = list(cr_base)
                    tmp[sales_idx] = M_rule
                    cr = tmp + win_part + [rule['name'], days, '是' if ok else '否', note]
                yield cr
                n += 1
                if on_progress and n % 5000 == 0:
                    on_progress(n)
...
def process_inv_check(in_fba, in_inv, out_path, on_progress=None):
    """总可用库存核对：FBA需求表 × 库存表，按 SKU编码 关联。
    取 FBA需求表每个 SKU 的「本地SKU总可用库存」，与 库存表按 SKU编码 汇总（跨仓库求和）得到的
    「可用库存」比对，逐 SKU 标记 一致 / 不一致 / 库存缺失。一个 SKU 在 FBA需求表出现多行时取首值
    （同一 SKU 的本地SKU总可用库存各地一致）。返回 (header, n, summary, preview)。
    """
    def fidx(hdr, *names):
        # 优先精确匹配，其次子串包含匹配（兼容列名略有出入的文件）
        for i, h in enumerate(hdr):
    ...
    # 1) 库存表：按 SKU编码 汇总 可用库存（跨仓库求和，得到「总可用库存」）
    ih, iit = _calamine_rows(in_inv) if _try_calamine(in_inv) else _openpyxl_rows(in_inv)
    i_sku_i = fidx(ih, 'SKU编码')
    i_avail = fidx(ih, '可用库存', '可用库存数量')
    if i_sku_i < 0:
        raise ValueError("库存表缺少 SKU编码 列（当前列：%s）" % ih)
    if i_avail < 0:
        raise ValueError("库存表缺少 可用库存 列（当前列：%s）" % ih)
    inv_total = {}   # sku -> 汇总可用库存（跨仓库）
    inv_rows = {}    # sku -> 出现行数（仓库数）
    for r in iit:
        s = r[i_sku_i]
        if s is None:
            continue
        ...
        if not s:
            continue
        inv_total[s] = inv_total.get(s, 0.0) + fnum(r[i_avail])
        inv_rows[s] = inv_rows.get(s, 0) + 1

    # 2) FBA需求表：取每个 SKU 的 本地SKU总可用库存（同一 SKU 各地值一致，取首值）
    fh, fit = _calamine_rows(in_fba) if _try_calamine(in_fba) else _openpyxl_rows(in_fba)
    i_sku_f = fidx(fh, 'SKU编码')
    i_local = fidx(fh, '本地SKU总可用库存', '本地SKU总可用')
    if i_sku_f < 0:
        raise ValueError("FBA需求表缺少 SKU编码 列（当前列：%s）" % fh)
    if i_local < 0:
        raise ValueError("FBA需求表缺少 本地SKU总可用库存 列（当前列：%s）" % fh)
    fba_local = {}   # sku -> 本地SKU总可用库存
    fba_rows = {}    # sku -> 出现行数
    for r in fit:
        s = r[i_sku_f]
        if s is None:
        ...
        if not s:
            continue
        fba_rows[s] = fba_rows.get(s, 0) + 1
        if s not in fba_local:
            fba_local[s] = fnum(r[i_local])

    out_header = ['SKU编码', 'FBA需求-本地SKU总可用库存', '库存表-总可用库存（按SKU汇总）',
                  '差值(FBA-库存)', '是否一致', 'FBA需求出现行数', '备注']
    rows = []
    n_total = n_consistent = n_inconsistent = n_missing = 0
    inv_only = len(inv_total) - len(set(inv_total) & set(fba_local))
    ...
    for s in fba_local:
        ...
        if s in inv_total:
            iv = inv_total[s]
            diff_v = round(fv - iv, 4)
            consistent = abs(diff_v) <= 1e-6
            if consistent:
                n_consistent += 1
                note = ''
            else:
                n_inconsistent += 1
                note = '本地SKU总可用库存 与 库存表可用库存 不一致'
        else:
            iv = ''
            diff_v = ''
            consistent = False
            ...
            note = '库存表无此SKU记录'
        row = [s,
               round(fv, 4) if isinstance(fv, (int, float)) else fv,
               round(iv, 4) if isinstance(iv, (int, float)) else iv,
               diff_v,
    ...
    # 排序：不一致(有库存但不等) → 库存缺失 → 一致；同组按 SKU 升序
    def rank(x):
        consistent = (x[4] == '是')
        missing = (x[2] == '')
        return (0 if (not consistent and not missing) else (1 if missing else 2), x[0])
    rows.sort(key=rank)
    ...
    if on_progress:
        on_progress(n_total)
    summary = {'fba_skus': n_total, 'matched': n_total - n_missing,
               'consistent': n_consistent, 'inconsistent': n_inconsistent,
               'inv_missing': n_missing, 'inv_only': inv_only}
    preview = rows[:200]
    return out_header, n_total, summary, preview


# （已移除：多余欧洲区销量核对明细导出 —— 页签③已替换为「总可用库存核对」）
...
class H(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if m:
            ...
            # 兼容写法：legacy filename 必须 latin-1 安全（纯 ASCII），
            # 现代浏览器优先使用 filename* 的 UTF-8 百分号编码版本（支持中文文件名）。
            safe = ''.join(ch if 32 <= ord(ch) < 128 else '_' for ch in fname)
            disp = "attachment; filename=\"%s\"; filename*=UTF-8''%s" % (safe, urllib.parse.quote(fname))
            self._send(200, data,
    ...
    def do_POST(self):
        ...
        if typ == 'fba':
            ...
            cols = {
                ...
                'd30': (qs_get(p, 'colD30') or '').strip(),
                'd60': (qs_get(p, 'colD60') or '').strip(),
                'd90': (qs_get(p, 'colD90') or '').strip(),
            }
        ...
        # ---- 总可用库存核对（FBA需求表 + 库存表，按 SKU编码 比对） ----
        if typ == 'inv_check':
            sid = qs_get(p, 'sid') or ''
            up = UPLOADS.get(sid, {})
            if not (up.get('fba') and up.get('stock')):
                self._json(400, {'error': '请先上传 FBA需求表 与 库存表'})
                return
            tid = uuid.uuid4().hex[:12]
            task = {
                'id': tid, 'status': 'running', 'type': 'inv_check', 'filename': 'inv_check',
                'n_rows': 0, 'header': None, 'preview_rows': None, 'summary': None,
                'error_msg': None, 'sid': sid,
                'download_name': '总可用库存核对结果.xlsx',
                'created': time.time(), 'finished': None,
            }
            with LOCK:
                TASKS[tid] = task
            threading.Thread(target=run_inv_check, args=(sid, task), daemon=True).start()
            self._json(202, {'id': tid, 'status': 'running', 'n_rows': 0})
            return
...
def run_fba(sid, task):
    try:
        ...
        # 页签②：销量原始文件不再单独处理输出，直接作为 lookup 源传给 process_ops
        hdr_o, n_o, meet, level_info = process_ops(ops_p, sales_p, out_ops, w3, w7, w14,
                                                    cols=task.get('cols'), on_progress=prog,
                                                    levels=task.get('levels'), cfg=cfg, mapping=mp)
        hdr_d, n_d, summary, diff_preview = process_diff(out_ops, fba_p, out_diff, cols=task.get('cols'),
                                                         cfg=cfg, mapping=mp)

        with LOCK:
            task['status'] = 'done'
            task['outputs'] = {'ops': out_ops, 'diff': out_diff}
            task['summary'] = summary
            task['n_rows'] = n_o
            task['header'] = hdr_o
            task['preview_rows'] = None
...
def run_inv_check(sid, task):
    try:
        up = UPLOADS.get(sid, {})
        fba_p = up.get('fba'); inv_p = up.get('stock')
        if not (fba_p and inv_p):
            raise ValueError("缺少上传文件（FBA需求表 / 库存表）")
        tid = task['id']
        out_path = os.path.join(HERE, '_inv_%s.xlsx' % tid)

        def prog(n):
            with LOCK:
                task['n_rows'] = n

        hdr, n, summary, preview = process_inv_check(fba_p, inv_p, out_path, on_progress=prog)
        with LOCK:
            task['status'] = 'done'
